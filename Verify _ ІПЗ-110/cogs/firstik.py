    ...
    def __init__(self, bot, author: disnake.Member, member: disnake.Member, limit, udal, embed):
        ...
        class tvink(disnake.ui.View):
            ...
            async def back2unban2z(this, _, button_interaction):
                ...
                embed.set_image(url = "https://cdn.discordapp.com/attachments/1167124886670544947/1171882191450546267/IMG_20231109_014051.png?ex=655e4b56&is=654bd656&hm=c8ba277b47a0c7e96e27241236b3a82f3f2163c847d64eb071cca6aefdaa615c&")
...
class veref_catalog(disnake.ui.View):
    ...
    async def back2unban2zxczxc(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        ...
        veref2 = interaction.guild.get_role(int(1155925453832470550))
        await self.member.add_roles(role)
        await self.member.add_roles(veref2)
    ...
    async def back2unban2zxczxc2(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        if interaction.author != self.author:
            await interaction.response.defer()
            return
        await interaction.response.defer()
...
class lx_clas2(disnake.ui.Select):
    ...
    async def callback(self, interaction: disnake.Interaction):
        ...
        veref = interaction.guild.get_role(int(config['unverify']))
        veref2 = interaction.guild.get_role(int(1155925453832470550))
        await self.member.remove_roles(veref2)
        await self.member.add_roles(role)
        await self.member.remove_roles(veref)
...
class buton(disnake.ui.View):
    ...
    @disnake.ui.button(style = ButtonStyle.green, label = "Верифікувати")
    async def back2unban2zxc(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        ...
        print(f"Кнопка Верифікувати виконалася за {execution_time} секунд")
    ...
    async def back2unban2zxcяяя(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        ...
        print(f"Кнопка Недопуск виконалася за {execution_time} секунд")




    @disnake.ui.button(style = ButtonStyle.grey, label = "Твінки")
    async def back2unban2(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        ...
        print(f"Кнопка Твінки виконалася за {execution_time} секунд")

    @disnake.ui.button(style = ButtonStyle.red, label = "Відміна")
    async def back2unban2zzxz(self, button: disnake.ui.Button, interaction: disnake.MessageInteraction):
        ...
        print(f"Кнопка Відміна виконалася за {execution_time} секунд")
...
class video(commands.Cog, name="video"):
    ...
    async def create(self, interaction, member: disnake.Member):
        ...
        if member == interaction.author:
            embed = disnake.Embed(title = "Верификация", description = f"{interaction.author.mention}, **Вы** не можете **проводить** операции **сами с собой.**",color =  0x2f3136).set_thumbnail(url = interaction.author.display_avatar)
        ...
        print(f"Команда verify виконалася за {execution_time} секунд")

    @commands.slash_command()
    async def support(self, interaction):
        pass
    @support.sub_command(name = "stats", description = "Статистика саппорта")
    async def stats(self, interaction):
        
        role = interaction.guild.get_role(int(config['support']))
        if role not in interaction.author.roles:
            embed = disnake.Embed(title = 'Статистика саппорта', description = f"{interaction.author.mention}, **Вы** не являетесь {role.mention}", color =  0x2f3136).set_thumbnail(url = interaction.author.display_avatar)
            await interaction.send(embed = embed, ephemeral = True)
            return
        find = coll3.find_one({"id": interaction.author.id})
        if find:
            timezxczxc = f"{find['online'] // 3600} ч. {(find['online'] // 60) % 60} мин."
            embed = disnake.Embed(title = f"Статистика — {interaction.author}", description = f"> {config['dot']}**Онлайн в прихожей:**\n```{timezxczxc}```",color =  0x2f3136).set_thumbnail(url = interaction.author.display_avatar)
            ...
            if coll3.count_documents({"id": interaction.author.id}) == 0:
                coll3.insert_one(post)
            fin2 = coll3.find_one({"id": interaction.author.id})
        ...
        for guild in self.bot.guilds:
            if guild.id == int(config['guild_id']):
                for channel in guild.voice_channels:
                    channelszxc = [1156058417488732211, 1156058652227149945, 1156058700591681586, 1156058750948479077, 1156058917231673425]
